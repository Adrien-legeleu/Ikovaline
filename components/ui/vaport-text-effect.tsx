...
export default function VaporizeTextCycle({
  ...
  tag = Tag.P,
  className = '',
}: VaporizeTextCycleProps & { className?: string }) {
...
const renderCanvas = ({
  ...
  // Récupère la couleur effective du parent (currentColor)
  let effectiveColor = framerProps.color ?? 'currentColor';
  if (effectiveColor === 'currentColor' && canvas.parentElement) {
    const computed = getComputedStyle(canvas.parentElement).color;
    effectiveColor = computed; // ex: "rgb(255, 255, 255)" en dark
  }
  ...
  const color = parseColor(effectiveColor);
