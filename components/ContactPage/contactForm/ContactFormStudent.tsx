...
export default function ContactFormStudent({ onSwitch }: { onSwitch: () => void }) {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        toast({ title: 'Message envoyé !', description: 'Nous revenons vers vous très vite.' });
        setFormData({ firstname: '', lastname: '', etude: '', email: '', tel: '', message: '', category: 'Étudiant' });
  ...
  return (
    ...
    >
      ...
      <p className="mt-3 mx-auto max-w-md text-center text-base leading-relaxed text-neutral-700 dark:text-neutral-300">
        Passionné(e) par le digital ? <b>Ikovaline recrute</b> des étudiants motivés sur des projets concrets pour la
      ...
      <form className="my-8" onSubmit={onSubmit}>
        <div className="mb-4 flex flex-col gap-2 md:flex-row">
          <LabelInputContainer>
            ...
            <Input id="firstname" required placeholder="Martin" value={formData.firstname} onChange={onChange('firstname')} className={fieldCls} />
          </LabelInputContainer>
          <LabelInputContainer>
            <Label htmlFor="lastname">Prénom</Label>
            <Input id="lastname" required placeholder="Lina" value={formData.lastname} onChange={onChange('lastname')} className={fieldCls} />
          </LabelInputContainer>
        </div>

        <LabelInputContainer className="mb-4">
          <Label htmlFor="email">Adresse mail</Label>
          <Input id="email" required type="email" placeholder="lina@exemple.com" value={formData.email} onChange={onChange('email')} className={fieldCls} />
        </LabelInputContainer>
        ...
        <LabelInputContainer className="mb-4">
          ...
          <Input id="etude" required placeholder="Master" value={formData.etude} onChange={onChange('etude')} className={fieldCls} />
        </LabelInputContainer>

        <LabelInputContainer className="mb-4">
          <Label htmlFor="message">Message</Label>
          <Textarea id="message" placeholder="Parlez-nous de vous…" value={formData.message} onChange={onChange('message')} className={cn(fieldCls, 'min-h-28')} />
        </LabelInputContainer>
